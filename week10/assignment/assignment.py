...
I feel as though I am on the right track, but I'm not sure exactly what direction
I need to take. I've been trying to reference some of the reading material, but ya.
"""
import random
...
def writter(total_items, shared, current, lock):
    while total_items < current:
        lock.acquire()
        shared[current] = total_items - current
        lock.release()
        time.sleep(.01)
        current += 1


def reader(shared, current, lock):
    read = 0
    while True:
        lock.acquire()
        i = shared[current]
...
def main():
    ...
    shared = smm.ShareableList(range(BUFFER_SIZE + 1))

    # TODO - Create any lock(s) or semaphore(s) that you feel you need
    lock = mp.Lock()

    # TODO - create reader and writer processes
    reader_p = mp.Process(target=reader, args=(shared, 0, lock))
    writer_p = mp.Process(target=writter, args=(
        items_to_send, shared, 0, lock))
